...
function generateAIAdvisorResponse(query) {
  ...
  if (lowercase.includes('dora gaps') || lowercase.includes('show dora gaps')) {
    const state = getState();
    const gaps = state.obligations.filter(ob => ob.status !== 'Compliant');
    let tableHtml = `
      <p>I have queried our compliance ledger for active DORA alignment gaps. Here are the items requiring immediate action:</p>
      <div style="overflow-x:auto; margin-top:10px; border:1px solid var(--border-color); border-radius:4px; max-width:100%;">
        <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.7rem;">
          <thead>
            <tr style="background:rgba(255,255,255,0.04); border-bottom:1px solid var(--border-color); color:var(--text-secondary); font-weight:700;">
              <th style="padding:6px 8px;">Article</th>
              <th style="padding:6px 8px;">Pillar</th>
              <th style="padding:6px 8px;">Compliance Requirement</th>
    ...
    `;
    gaps.forEach(g => {
      const badgeStyle = g.status === 'Non-Compliant' ? 'background:rgba(239,68,68,0.1); color:#ef4444; border:1px solid rgba(239,68,68,0.2);' : 'background:rgba(234,179,8,0.1); color:#eab308; border:1px solid rgba(234,179,8,0.2);';
      tableHtml += `
        <tr style="border-bottom:1px solid var(--border-color);">
          <td style="padding:6px 8px; color:var(--text-primary);"><b>${g.article}</b></td>
          <td style="padding:6px 8px; color:var(--text-secondary);">${g.pillar}</td>
          <td style="padding:6px 8px; color:var(--text-muted);">${g.title}: ${g.description}</td>
          <td style="padding:6px 8px;"><span style="font-size:0.56rem; padding:2px 6px; border-radius:4px; font-weight:700; ${badgeStyle}">${g.status}</span></td>
        </tr>
      `;
    });
    tableHtml += `
  ...
  if (lowercase.includes('support payments') || lowercase.includes('suppliers support payments')) {
    const state = getState();
    const pmtSrv = state.services.find(s => s.name.includes('Payments'));
    let suppliersHtml = `
      <p>I have mapped the operational dependencies for our critical payments services. The following fourth-party subcontractors and primary suppliers directly support <strong>${pmtSrv ? pmtSrv.name : 'IBS Payments Processing'}</strong>:</p>
      <div style="overflow-x:auto; margin-top:10px; border:1px solid var(--border-color); border-radius:4px; max-width:100%;">
        <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.7rem;">
          <thead>
            <tr style="background:rgba(255,255,255,0.04); border-bottom:1px solid var(--border-color); color:var(--text-secondary); font-weight:700;">
              <th style="padding:6px 8px;">Vendor Partner</th>
              <th style="padding:6px 8px;">Risk Tier</th>
              <th style="padding:6px 8px;">Role Mapped</th>
              <th style="padding:6px 8px;">Compliance Rating</th>
            </tr>
          </thead>
          <tbody>
    `;
    ...
    if (aws) {
      suppliersHtml += `
        <tr style="border-bottom:1px solid var(--border-color);">
          <td style="padding:6px 8px; color:var(--text-primary);"><b>${aws.name}</b></td>
          <td style="padding:6px 8px; color:#ef4444; font-weight:700;">Critical</td>
          <td style="padding:6px 8px; color:var(--text-secondary);">Primary Cloud Host (us-east-1a containers)</td>
          <td style="padding:6px 8px; color:#eab308; font-weight:700;">${aws.complianceScore}% (Gaps)</td>
        </tr>
      `;
      aws.subcontractors.forEach(sub => {
        suppliersHtml += `
          <tr style="border-bottom:1px solid var(--border-color); background:rgba(255,255,255,0.01);">
            <td style="padding:6px 8px; color:var(--text-muted);">↳ <em>${sub.name} (Subprocessor)</em></td>
            <td style="padding:6px 8px; color:var(--text-muted);">Nth-Party</td>
            <td style="padding:6px 8px; color:var(--text-muted);">${sub.role}</td>
            <td style="padding:6px 8px; color:var(--text-muted);">Flow-down Bound</td>
          </tr>
        `;
      });
    }
  ...
  if (lowercase.includes('lack testing') || lowercase.includes('critical services lack testing')) {
    const state = getState();
    let untestedHtml = `
      <p>I have audited our critical business services against our operational resilience test register. The following systems require testing reviews:</p>
      <div style="overflow-x:auto; margin-top:10px; border:1px solid var(--border-color); border-radius:4px; max-width:100%;">
        <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.7rem;">
          <thead>
            <tr style="background:rgba(255,255,255,0.04); border-bottom:1px solid var(--border-color); color:var(--text-secondary); font-weight:700;">
              <th style="padding:6px 8px;">Critical Business Service</th>
              <th style="padding:6px 8px;">Criticality</th>
              <th style="padding:6px 8px;">Last DR Test Date</th>
    ...
    state.services.forEach(srv => {
      ...
      untestedHtml += `
        <tr style="border-bottom:1px solid var(--border-color);">
          <td style="padding:6px 8px; color:var(--text-primary);"><b>${srv.name}</b></td>
          <td style="padding:6px 8px; color:#ef4444; font-weight:700;">${srv.criticality}</td>
          <td style="padding:6px 8px; color:var(--text-secondary);">${testDate}</td>
          <td style="padding:6px 8px;"><span style="font-size:0.56rem; padding:2px 6px; border-radius:4px; font-weight:700; ${statusStyle}">${statusText}</span></td>
        </tr>
      `;
    });
    untestedHtml += `
  ...
  if (lowercase.includes('generate board report') || lowercase.includes('board report')) {
    const state = getState();
    const activeInc = state.incidents ? state.incidents.filter(i => i.status !== 'Resolved').length : 0;
    const totalGaps = state.obligations ? state.obligations.filter(o => o.status !== 'Compliant').length : 0;
    const testCount = state.tests ? state.tests.length : 0;

    return `
      <div style="background: rgba(255,255,255,0.01); border: 1px solid var(--border-color); border-radius: 6px; padding: 15px;">
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom: 1px dashed var(--border-color); padding-bottom: 8px;">
          <h4 style="margin: 0; font-size: 0.85rem; text-transform: uppercase; color: var(--color-cyan); font-weight: 700;">Executive DORA Board Briefing Report</h4>
          <span style="font-size:0.6rem; color:var(--text-muted); font-family:monospace;">Date: 2026-07-20</span>
        </div>
        
        <div style="margin-top: 12px; display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; font-size: 0.7rem;">
          <div style="background: rgba(255,255,255,0.01); border: 1px solid var(--border-color); padding: 8px; border-radius: 4px;">
            <span style="color: var(--text-muted); display: block; font-size: 0.58rem; text-transform: uppercase;">DORA Compliance Score</span>
            <strong style="font-size: 1.2rem; color: #eab308; display: block; margin-top: 2px;">82.5%</strong>
            <span style="color: var(--text-secondary); font-size: 0.58rem;">${totalGaps} Active Gaps Mapped</span>
          </div>

          <div style="background: rgba(255,255,255,0.01); border: 1px solid var(--border-color); padding: 8px; border-radius: 4px;">
            <span style="color: var(--text-muted); display: block; font-size: 0.58rem; text-transform: uppercase;">Resilience Confidence</span>
            <strong style="font-size: 1.2rem; color: #10b981; display: block; margin-top: 2px;">92.0%</strong>
            <span style="color: var(--text-secondary); font-size: 0.58rem;">${testCount} Tests Logged</span>
          </div>
        </div>

        <div style="margin-top:12px; font-size:0.7rem; line-height:1.4; color:var(--text-secondary);">
          <strong>Key Highlights:</strong>
          <ul style="padding-left:15px; margin: 4px 0 0 0; display:flex; flex-direction:column; gap:4px;">
            <li>⚠️ <strong>Nth-Party Risk Alert:</strong> Subcontractor concentration identified at <strong>Cloudflare</strong> (shared dependency between AWS us-east-1a and Salesforce).</li>
            <li>📉 <strong>Incident Logging:</strong> Mapped <strong>${activeInc} active operational incidents</strong> currently in remediation workflow.</li>
            <li>🚀 <strong>BAS Simulation:</strong> Automated TIBER-EU campaign scans completed on Slack & Salesforce. Boundary controls evaluated as secure.</li>
          </ul>
        </div>
      </div>
    `;
  ...
  if (lowercase.includes('resilience posture') || lowercase.includes('summarise resilience posture') || lowercase.includes('summarize resilience posture')) {
    const state = getState();
    const criticalSuppliers = Object.values(state.suppliers).filter(s => s.riskTier === 'Critical').length;
    const activeGaps = state.actions.filter(a => a.status !== 'Closed').length;

    return `
      <p>Here is the executive summary of the Cypher Vantage operational resilience posture:</p>
      <div style="background:rgba(255,255,255,0.01); border:1px solid var(--border-color); padding:10px; border-radius:6px; font-size:0.72rem; line-height:1.45;">
        🎯 <strong>Resilience Readiness Index:</strong> Mapped at <strong>92% confidence rating</strong> based on incremental failover checks.<br/>
        🏢 <strong>ICT Vendor Mappings:</strong> monitoring <strong>${criticalSuppliers} critical tier vendor accounts</strong>.<br/>
        🚨 <strong>Remediation Pipeline:</strong> <strong>${activeGaps} active gaps</strong> are currently tracked in the Follow-up Action register (e.g. AWS missing BCP evidence and Infosys encryption policy mismatch).<br/>
        🛡️ <strong>AI Governance Status:</strong> 4 LLM models catalogued, outbound DLP Proxy active, and zero prompt exfiltrations observed.
      </div>
    `;
  }
...
Risk Director, Cypher Vantage Compliance Team
  ...
  return `
    ...
    <p>As the Cypher Vantage AI Copilot, I can directly query the live database state to assist you.</p>
    <p>Try asking me:</p>
    <ul>
      <li><a href="#" onclick="triggerPresetQuery('Show DORA gaps'); return false;" style="color:var(--color-cyan); font-weight:600;">"Show DORA gaps"</a></li>
      <li><a href="#" onclick="triggerPresetQuery('Which suppliers support Payments?'); return false;" style="color:var(--color-cyan); font-weight:600;">"Which suppliers support Payments?"</a></li>
      <li><a href="#" onclick="triggerPresetQuery('Which critical services lack testing?'); return false;" style="color:var(--color-cyan); font-weight:600;">"Which critical services lack testing?"</a></li>
      <li><a href="#" onclick="triggerPresetQuery('Generate Board Report'); return false;" style="color:var(--color-cyan); font-weight:600;">"Generate Board Report"</a></li>
      <li><a href="#" onclick="triggerPresetQuery('Summarise resilience posture'); return false;" style="color:var(--color-cyan); font-weight:600;">"Summarise resilience posture"</a></li>
    </ul>
  `;
}
